...
void getContours(cv::Mat imgDilate, cv::Mat img)
{
	...
	for (int i = 0; i < contours.size(); i++)
	{
		...
		float circularity = 4 * 3.14 * area / (peri * peri);
		if (circularity > 0.3 && circularity < 0.7)//using concept of circularity to see how circular is the found contour
...
int main()
{
	std::string path = "Resources/virat_test_pothole.mp4";
